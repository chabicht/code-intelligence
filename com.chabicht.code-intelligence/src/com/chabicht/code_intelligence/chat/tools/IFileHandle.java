...
/**
 * A lightweight abstraction for file handles that supports both real Eclipse
 * IFile instances and virtual files that exist only in memory (pending
 * creation).
 * 
 * This interface provides only the minimal set of methods needed by the AI
 * tools, avoiding the complexity of implementing the full IFile interface.
 */
public interface IFileHandle {
	 ...
	 * 
	 * @return true for existing real files, false for virtual files or non-existent
	 *         files
	 */
	boolean exists();
	 ...
	 * 
	 * @return the full path as a string, e.g.,
	 *         "/ProjectName/src/com/example/MyClass.java"
	 */
	String getFullPath();
