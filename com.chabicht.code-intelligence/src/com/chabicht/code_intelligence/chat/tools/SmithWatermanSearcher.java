...
public class SmithWatermanSearcher {
	 ...
	 * using the Smith-Waterman algorithm. Considers whitespace normalization
	 * similar to ChunkSearcher.
	 *
	 * @param patternToSearch The pattern text.
	 * @param textToSearchIn  The text to search within.
	 * @return An array [startIndex, endIndex] of the matching region in
	 *         `textToSearchIn` (original coordinates), or null if no meaningful
	 *         match is found. Indices are [inclusiveStart, exclusiveEnd].
	 */
	public int[] findMatchingRegion(String patternToSearch, String textToSearchIn) {
		...
		// Normalize text (collapse spaces, but DO NOT trim overall, to preserve
		// original mapping context)
		String normText = textToSearchIn.replaceAll("\\s+", " ");

		if (normPattern.isEmpty()) {
			// An empty pattern cannot be meaningfully searched by Smith-Waterman in this
			// context.
			// ChunkSearcher's findOffsetsByPattern throws if originalText.trim().isEmpty().
			// If originalText was not empty but normPattern is (e.g. " "), this is the
			// place to catch it.
			return null;
		}
		...
		// dp[i][j] will be the score of the best alignment ending at normPattern[i-1]
		// and normText[j-1]
		...
		for (int i = 1; i <= n; i++) {
			for (int j = 1; j <= m; j++) {
				int charScore = (normPattern.charAt(i - 1) == normText.charAt(j - 1)) ? config.matchScore
		...
		while (current_i > 0 && current_j > 0 && dp[current_i][current_j] > 0) {
			...
			int charScore = (normPattern.charAt(current_i - 1) == normText.charAt(current_j - 1)) ? config.matchScore
					: config.mismatchPenalty;
			...
			// The loop continues as long as score > 0. When it stops, current_j is one step
			// *before* the start.
			// So, the actual start index (0-based) in normText is current_j (after loop,
			// before this comment)
		}
		// After loop, current_j is the 0-based start index in normText if match started
		// from dp[...][current_j+1]
		// Or, if the path went straight to a 0, current_j is where the alignment
		// effectively started.
		...
		// Ensure start is not after end, which can happen if mapping is tricky or match
		// is zero-length at end of whitespace
		if (finalStart > finalEnd && normStart_j == normEnd_j) { // typically for zero-length matches
			finalEnd = finalStart;
		} else if (finalStart > finalEnd) {
			// This would be unusual and might indicate an issue or a very strange mapping
			// result
	 ...
	 * (Copied from ChunkSearcher for self-containment, consider making it a shared
	 * utility)
	 *
	...
	private static int mapS1ToOriginalOffset(String originalText, int s1Offset) {
		if (s1Offset == 0) {
			// Optimization: If originalText starts with whitespace, s1 (normText) might
			// have its content
			// starting at index 0, but this 0 in s1 corresponds to the first non-whitespace
			// char in originalText.
			// However, the current ChunkSearcher's mapS1ToOriginalOffset returns 0 for
			// s1Offset 0.
		...
		while (originalIdx < originalText.length() && currentS1Offset < s1Offset) {
			...
			if (Character.isWhitespace(currentChar)) {
				...
				// We increment currentS1Offset only when we transition from whitespace to
				// non-whitespace
				...
				// Check if the previous char in s1 would have been a space due to this
				// whitespace
				if (originalIdx > 0) { // currentS1Offset is based on s1, which has single spaces
					...
					// Let's refine: consume all whitespace in originalText, then increment s1
					// offset.
					char prevOriginalChar = (originalIdx > 1) ? originalText.charAt(originalIdx - 2) : ' '; // Hypothetical
																											// char
																											// before
																											// this
																											// whitespace
					...
					// The critical part is that `normText` (s1) is
					// `textToSearchIn.replaceAll("\\s+", " ")`
					...
					currentS1Offset++; // This leading whitespace block becomes one space in s1 (or empty if s1 is
										// trimmed, but normText is not)
				}
			} else {
		...
		// If s1Offset points beyond the characters represented by non-whitespace
		// sequences in originalText
		// (e.g., s1Offset refers to a trailing space in s1 that came from trailing
		// whitespace in originalText),
		// originalIdx should be at the end of originalText or at the start of the final
		// whitespace block.
		// The loop condition `currentS1Offset < s1Offset` ensures we count up to the
		// desired s1 position.
		...
		// Example: original="a b", s1="a b". s1Offset=1 (points to 'a').
		// currentS1Offset becomes 1, originalIdx becomes 1. Returns 1.
		// s1Offset=2 (points to ' '). currentS1Offset becomes 2, originalIdx becomes 3
		// (after ' '). Returns 3.
		// This is slightly different from ChunkSearcher's mapS1ToOriginalOffset. Let's
		// use its exact copy.
