...
public interface IResourceAccess {
	 ...
	 *
	 * @param filePath The complete path (relative to workspace root) for the new
	 *                 file.
	 * @param content  The content to write into the new file.
	 * @return A {@link CreateFileTool.CreateFileResult} indicating success or
	 *         failure.
	 */
	CreateFileResult createFileInWorkspace(String filePath, String content);

	/**
	 * Finds a file handle by name, including virtual/pending files. This method
	 * should be used instead of findFileByNameBestEffort when virtual file support
	 * is needed.
	 * 
	 * The search strategy is: 1. Check for exact match in virtual files 2. Check
	 * for name match in virtual files 3. Delegate to findFileByNameBestEffort for
	 * real files
	 * 
	...
	/**
	 * Gets document content for a file handle (real or virtual). For virtual files,
	 * creates a new Document from the content. For real files, delegates to
	 * getDocumentAndConnect.
	 * 
